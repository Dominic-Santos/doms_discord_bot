...
class TestBotDecklist(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_bot_signup_sheet(
        ...
        assert mock_ctx.last_response == "Output channel is not set for this server."
        ...
        await b.test_output_channel(mock_ctx)
        assert mock_ctx.last_response == "Output channel not found. Please set it again."

        mock_bot.get_channel.return_value = mock_ctx
        await b.test_output_channel(mock_ctx)
        assert mock_ctx.last_response == "Test message sent to the output channel!"
        assert mock_ctx.last_send == "This is a test message from the bot!"

        assert b.check_sign_up_sheet() in [True, False]

        assert b.check_limitless_url("https://my.limitlesstcg.com/builder?i=abc123abc")
        assert b.check_limitless_url("y.limitlesstcg.com/builder?i=abc123abc") is False
        ...
        assert mock_ctx.last_response == "Decklist is not valid: Invalid Limitless URL."

        await b.decklist_check(mock_ctx, "https://my.limitlesstcg.com/builder?i=abc123abc")
        ...
        assert mock_ctx.last_response == "Legal cards are not loaded. Please try again later."
        ...
        await b.tournament_signup(mock_ctx, "test person", 1234, 1990, "https://my.limitlesstcg.com/builder?i=abc123abc")
        assert mock_ctx.last_response == "Sign-up sheet is not available. Please try again later."

        mock_sign_up_sheet.return_value = True
        await b.tournament_signup(mock_ctx, "test person", 1234, 1990, "https://limitlesstcg.com/builder?i=abc123abc")
        assert mock_ctx.last_response == "Decklist is not valid: Invalid Limitless URL."
