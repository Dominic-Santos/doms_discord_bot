...
from src.pkmncards import get_legal_card_list, save_cards_to_file, get_legal_cards, get_card_text

@patch('src.pkmncards.Driver')
def test_get_card_text(mock_driver):
    mock_driver_instance = mock_driver.return_value
    mock_driver_instance.cdp.find_visible_elements.return_value = [MagicMock(text="Card text here")]

    card_text = get_card_text("https://example.com/card")

    mock_driver.assert_called_once_with(uc=True, locale_code="en", ad_block=True)
    mock_driver_instance.uc_activate_cdp_mode.assert_called_once_with("https://example.com/card")
...
def test_get_legal_cards(mock_save, mock_get):
    mock_get.return_value = [{"set": "Set 1", "number": "001", "name": "Card 1", "type": "Type 1", "color": "Color 1", "rarity": "Rare", "link": "https://example.com/card1"}]
...
def test_get_legal_card_list(mock_driver):
                        ...
                        MagicMock(children=[
                            MagicMock(children=[
                                MagicMock(children=[
                                    ...
                                    MagicMock(text="001"),
                                    MagicMock(text="Card 1", children=[{"href": "https://example.com/card1"}]),
    ...
    assert mock_elements.counter == 1  # Ensure pagination was attempted